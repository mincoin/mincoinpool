...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        print 'mincoinpool (version %s)' % (p2pool.__version__,)
        print
        
        @defer.inlineCallbacks
        def connect_p2p():
            # connect to mincoind over mincoin-p2p
            print '''Testing mincoind P2P connection to '%s:%s'...''' % (args.mincoind_address, args.mincoind_p2p_port)
            factory = bitcoin_p2p.ClientFactory(net.PARENT)
            reactor.connectTCP(args.mincoind_address, args.mincoind_p2p_port, factory)
            def long():
                print '''    ...taking a while. Common reasons for this include all of mincoind's connection slots being used...'''
        ...
        if args.testnet: # establish p2p connection first if testnet so mincoind can work without connections
            factory = yield connect_p2p()
        
        # connect to mincoind over JSON-RPC and do initial getmemorypool
        url = '%s://%s:%i/' % ('https' if args.mincoind_rpc_ssl else 'http', args.mincoind_address, args.mincoind_rpc_port)
        print '''Testing mincoind RPC connection to '%s' with username '%s'...''' % (url, args.mincoind_rpc_username)
        bitcoind = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(args.mincoind_rpc_username + ':' + args.mincoind_rpc_password)), timeout=30)
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            if address is not None:
                ...
                if not res['isvalid'] or not res['ismine']:
                    print '    Cached address is either invalid or not controlled by local mincoind!'
                    address = None
            
            if address is None:
                print '    Getting payout address from mincoind...'
                address = yield deferral.retry('Error getting payout address from mincoind:', 5)(lambda: bitcoind.rpc_getaccountaddress('mincoinpool'))()
            ...
            for i in range(args.numaddresses):
                address = yield deferral.retry('Error getting a dynamic address from mincoind:', 5)(lambda: bitcoind.rpc_getnewaddress('mincoinpool'))()
        ...
        print 'Joining mincoinpool network using port %i...' % (args.mincoinpool_port,)
        ...
        connect_addrs = set()
        for addr_df in map(parse, args.mincoinpool_nodes):
            try:
        ...
        node.p2p_node = p2pool_node.P2PNode(node,
            port=args.mincoinpool_port,
            max_incoming_conns=args.mincoinpool_conns,
            addr_store=addrs,
            connect_addrs=connect_addrs,
            desired_outgoing_conns=args.mincoinpool_outgoing_conns,
            advertise_ip=args.advertise_ip,
            external_ip=args.mincoinpool_external_ip,
        )
        node.p2p_node.start()
                ...
                while True:
                    try:
                        ...
                        if is_lan:
                            ...
                            yield pm._upnp.add_port_mapping(lan_ip, args.mincoinpool_port, args.mincoinpool_port, 'mincoinpool', 'TCP')
        ...
        if args.donation_percentage > 2.6:
            print '''Donating %.1f%% of work towards MincoinPool's development. Thanks for the tip!''' % (args.donation_percentage,)
        elif args.donation_percentage < 2.4:
            print '''Donating %.1f%% of work towards MincoinPool's development. Please donate to encourage further development of MincoinPool!''' % (args.donation_percentage,)
        else:
            print '''Donating %.1f%% of work towards MincoinPool's development. Thank you!''' % (args.donation_percentage,)
        ...
        if args.irc_announce:
            ...
            class IRCClient(irc.IRCClient):
                nickname = 'mincoinpool%02i' % (random.randrange(100),)
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    this_str = 'MincoinPool: %i shares in chain (%i verified/%i total) Peers: %i (%i incoming)' % (
                        height,
...
def run():
    ...
    parser = fixargparse.FixedArgumentParser(description='mincoinpool (version %s)' % (p2pool.__version__,), fromfile_prefix_chars='@')
    parser.add_argument('--version', action='version', version=p2pool.__version__)
    parser.add_argument('--net',
        help='use specified network (default: mincoin)',
        action='store', choices=sorted(realnets), default='mincoin', dest='net_name')
    ...
    parser.add_argument('-a', '--address',
        help='generate payouts to this address (default: <address requested from mincoind>), or (dynamic)',
    ...
    parser.add_argument('--coinbtext',
        ...
        type=str, action='append', default=['/MincoinPool/'], dest='coinb_texts')
    parser.add_argument('--give-author', metavar='DONATION_PERCENTAGE',
        help='donate this percentage of work towards the development of mincoinpool (default: 2.5)',
        type=float, action='store', default=2.5, dest='donation_percentage')
    ...
    parser.add_argument('--irc-announce',
        help='announce any blocks found on irc://irc.freenode.net/#mincoinpool',
        action='store_true', default=False, dest='irc_announce')
    
    p2pool_group = parser.add_argument_group('mincoinpool interface')
    p2pool_group.add_argument('--mincoinpool-port', metavar='PORT',
        ...
        type=int, action='store', default=None, dest='mincoinpool_port')
    p2pool_group.add_argument('-n', '--mincoinpool-node', metavar='ADDR[:PORT]',
        help='connect to existing mincoinpool node at ADDR listening on port PORT (defaults to default mincoinpool P2P port) in addition to builtin addresses',
        type=str, action='append', default=[], dest='mincoinpool_nodes')
    parser.add_argument('--disable-upnp',
        help='''don't attempt to use UPnP to forward mincoinpool's P2P port from the Internet to this computer''',
    ...
    p2pool_group.add_argument('--max-conns', metavar='CONNS',
        ...
        type=int, action='store', default=40, dest='mincoinpool_conns')
    p2pool_group.add_argument('--outgoing-conns', metavar='CONNS',
        help='outgoing connections (default: 6)',
        type=int, action='store', default=6, dest='mincoinpool_outgoing_conns')
    p2pool_group.add_argument('--external-ip', metavar='ADDR[:PORT]',
        ...
        type=str, action='store', default=None, dest='mincoinpool_external_ip')
    ...
    worker_group.add_argument('-f', '--fee', metavar='FEE_PERCENTAGE',
        help='''charge workers mining to their own mincoin address (by setting their miner's username to a mincoin address) this percentage fee to mine on your mincoinpool instance. Amount displayed at http://127.0.0.1:WORKER_PORT/fee (default: 0)''',
        type=float, action='store', default=0, dest='worker_fee')
    
    bitcoind_group = parser.add_argument_group('mincoind interface')
    bitcoind_group.add_argument('--mincoind-config-path', metavar='MINCOIND_CONFIG_PATH',
        help='custom configuration file path (when mincoind -conf option used)',
        type=str, action='store', default=None, dest='mincoind_config_path')
    bitcoind_group.add_argument('--mincoind-address', metavar='MINCOIND_ADDRESS',
        help='connect to this address (default: 127.0.0.1)',
        type=str, action='store', default='127.0.0.1', dest='mincoind_address')
    bitcoind_group.add_argument('--mincoind-rpc-port', metavar='MINCOIND_RPC_PORT',
        help='''connect to JSON-RPC interface at this port (default: %s <read from mincoin.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.RPC_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='mincoind_rpc_port')
    bitcoind_group.add_argument('--mincoind-rpc-ssl',
        help='connect to JSON-RPC interface using SSL',
        action='store_true', default=False, dest='mincoind_rpc_ssl')
    bitcoind_group.add_argument('--mincoind-p2p-port', metavar='MINCOIND_P2P_PORT',
        help='''connect to P2P interface at this port (default: %s <read from mincoin.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.P2P_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='mincoind_p2p_port')
    bitcoind_group.add_argument(metavar='MINCOIND_RPCUSERPASS',
        help='mincoind RPC interface username, then password, space-separated (only one being provided will cause the username to default to being empty, and none will cause MincoinPool to read them from mincoin.conf)',
        type=str, action='store', default=[], nargs='*', dest='mincoind_rpc_userpass')
    ...
    if len(args.mincoind_rpc_userpass) > 2:
        parser.error('a maximum of two arguments are allowed')
    args.mincoind_rpc_username, args.mincoind_rpc_password = ([None, None] + args.mincoind_rpc_userpass)[-2:]
    
    if args.mincoind_rpc_password is None:
        conf_path = args.mincoind_config_path or net.PARENT.CONF_FILE_FUNC()
        if not os.path.exists(conf_path):
            parser.error('''Mincoin configuration file not found. Manually enter your RPC password.\r\n'''
                '''If you actually haven't created a configuration file, you should create one at %s with the text:\r\n'''
                '''\r\n'''
                '''server=1\r\n'''
                '''rpcpassword=%x\r\n'''
                '''\r\n'''
                '''Keep that password secret! After creating the file, restart Mincoin.''' % (conf_path, random.randrange(2**128)))
        ...
        for conf_name, var_name, var_type in [
            ('rpcuser', 'mincoind_rpc_username', str),
            ('rpcpassword', 'mincoind_rpc_password', str),
            ('rpcport', 'mincoind_rpc_port', int),
            ('port', 'mincoind_p2p_port', int),
        ]:
        ...
        if 'rpcssl' in contents and contents['rpcssl'] != '0':
            args.mincoind_rpc_ssl = True
        if args.mincoind_rpc_password is None:
            parser.error('''Mincoin configuration file didn't contain an rpcpassword= line! Add one!''')
    
    if args.mincoind_rpc_username is None:
        args.mincoind_rpc_username = ''
    
    if args.mincoind_rpc_port is None:
        args.mincoind_rpc_port = net.PARENT.RPC_PORT
    
    if args.mincoind_p2p_port is None:
        args.mincoind_p2p_port = net.PARENT.P2P_PORT
    
    if args.mincoinpool_port is None:
        args.mincoinpool_port = net.P2P_PORT
    
    if args.mincoinpool_outgoing_conns > 10:
